...
`,description:"<p>Basic drawer.</p>",style:`[data-theme='compact'] .ant-drawer-body p {
  margin-bottom: 0;
}`}},{demo:{id:"components-drawer-demo-placement"},previewerProps:{title:"Custom Placement",filename:"components/drawer/demo/placement.tsx",jsx:`import { Button, Drawer, Radio, Space } from 'antd';
import { useState } from 'react';
const App = () => {
  const [open, setOpen] = useState(false);
...
`,description:"<p>Open a new drawer on top of an existing drawer to handle multi branch tasks.</p>",style:`[data-theme="dark"] .site-multi-level-drawer-footer {
  ...
  background: #1f1f1f;
}`}},{demo:{id:"components-drawer-demo-size"},previewerProps:{title:"Presetted size",filename:"components/drawer/demo/size.tsx",jsx:`import { Button, Drawer, Space } from 'antd';
import { useState } from 'react';
const App = () => {
  const [open, setOpen] = useState(false);
